...
library(tidyverse)
library(haven)
base_2004_2 <- read_dta("base_2004_2004.dta")
```
...
# a voir si on veut inclure ces nouvelles variables car peut-etre trop liees a la mobilite (on a 2 emplois si on part deja en 2004 etc)
```

### First attempt with multinom

```{r, eval=FALSE}
model<-multinom(stay_go~ age +age2 + netnetr + sal_autre + nb_emploi_autre + dp + xp + nbheur + nbsa_ent + nes5+sx  + cat + sect+ce+ statut+av +avr+commune +  region +regt+departement+ entsir + tain + cs1 + msb_ent + a38,   data=base_2004_22, maxit = 1000)
```
...
### \*\* l'iia n'est pas v?rifi? d'o? le Nested Logit Export vers stata\*\*
...
comparisons
#Ajuster le mod?le logit embo?t? avec nestedLogit :+ sal_autre + nb_emploi_autre entsir nbheur nbsa_ent y revenir 
model_nested <- nestedLogit(
  stay_go ~ age +age2 +    xp   + nes5+sx + dp+ netnetr + sal_autre + nb_emploi_autre  + cat + sect+ce +commune +  region +regt+departement + tain + cs1 + msb_ent + A17,
...
write.csv(data.frame(Predictor = rownames(coefs), Wald_Stat = wald_stat, p_value = wald_pvalue), paste0("wald_2004", dich , ".csv"))
  
}
```

c'es globalement significatif grace au LR test
...
```{r,eval = FALSE}
pdf(file = 'nested_stay_go_level_1_2004_coefs.pdf')
...
test_summary <- as.data.frame(summary(model_nested$models$partir)$coefficients)
write.csv(test_summary, "summary_level_2_nest_2_2004.csv")
```

```{r,eval = FALSE}

pdf(file = 'nested_stay_go_level_2_nid2_2004_coefs.pdf')
ggcoef_model(model_nested$models$partir, 
             exponentiate=TRUE,
             include=c("age", "age2", "netnetr","sal_autre", "nb_emploi_autre", "xp","sx","ce", "cs1","commune","region","departement"),
             colour=NULL)+
  scale_y_discrete(
    labels=c("netnetr"="Wage in constant euro",
             "sal_autre"= "Wages from other jobs",
...
write.csv(table_1, "table_1_level_2_nest_1_2004.csv")
...
write.csv(table_2, "table_2_level_2_nest_1_2004.csv")  
...
table_3 <- d %>% 
  ...
  summarise(
    n=n(), .groups = "drop") %>% 
  mutate(pourcentage=round(100*n/sum(n),2))
write.csv(table_3, "table_3_level_2_nest_1_2004.csv")
