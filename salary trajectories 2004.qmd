---
title: "Salary trajectories 2004"
format: html
editor: visual
---
...
library(haven)
library(tidyverse)
base_sal_2004 <- read_dta("base_suivi_2008.dta")
...
base_sal_2004<-base_sal_2004 %>% 
  mutate(stay_go = stay_2005_bin + stay_2008_bin,
         stay_go= as.factor(ifelse(is.na(stay_go) & !is.na(stay_2008_bin), 3, stay_go))) %>% 
...
pdf(file = 'salaires_desc_2004.pdf')
...
```{r}
#combien on des jours de paye en plus de l'emploi au salire max
testit <- sal_unique %>%
  mutate(dp_reste = dp_tot - dp) %>%
  filter(dp_reste >0)
summary(testit$dp_reste)
testit
```
...
pdf(file = 'salary_trajectory_2004_coefs.pdf')
